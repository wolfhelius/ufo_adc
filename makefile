...
# compiler
CFLAGS=-DF_CPU=$(F_CPU) -D$(DEBUG_OPTIONS) -std=c99 -I. $(INC) -g \
    -mmcu=$(MCU) -O$(OPTLEVEL) \
    -fpack-struct -fshort-enums -funsigned-bitfields -funsigned-char    \
    -Wall -Wstrict-prototypes -Wa,-ahlms=$(firstword \
    $(filter %.lst, $(<:.c=.lst)))
...
# linker
LDFLAGS=-lm -Wl,-u,vfprintf -lprintf_flt -lm \
    -Wl,-Map,$(TRG).map -mmcu=$(MCU) $(LIBS)
...
writeflash: hex
	$(AVRDUDE) -c $(AVRDUDE_PROGRAMMERID) $(AUTOERASE)  \
...
term: writeflash
	picocom -b 9600 --imap lfcrlf -i -l $(AVRDUDE_PORT)
#   microcom -s9600 -p $(AVRDUDE_PORT)
